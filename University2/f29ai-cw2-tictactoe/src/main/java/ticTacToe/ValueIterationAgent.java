...
public class ValueIterationAgent extends Agent {
	...
	public void iterate() {
		...
		while (iterator < k) {
			for (Game state : valueFunction.keySet()) {
				...
				if (state.isTerminal()) {
					valueFunction.replace(state, 0.0);
					continue;
				}
				...
				for (Move action : state.getPossibleMoves()) {
					...
					for (TransitionProb probability : mdp.generateTransitions(state, action)) {
						...
						double tempCalc = x.localReward + discount * valueFunction.get(x.sPrime);
				...
				valueFunction.replace(state, maximum);
	...
	public Policy extractPolicy() {
		...
		for (Game state : valueFunction.keySet()) {
			...
			extractedPolicy.policy.put(state, extractMove);
